...
- (nonnull instancetype)initWithCentralManager:(CBCentralManager * _Nonnull)centralManager target:(CBPeripheral * _Nonnull)target OBJC_DESIGNATED_INITIALIZER SWIFT_DEPRECATED_MSG("Use init(target: CBPeripheral) instead.");
/// Creates the DFUServiceInitializer that will allow to send an update to the given peripheral.
/// The DFU service will automatically connect to the device, if not connected already,
/// check if it has required DFU service (return a delegate callback if does not have such),
/// jump to the DFU Bootloader mode if necessary, and perform the DFU. Proper delegate methods
...
/// returns:
/// The initiator instance
- (nonnull instancetype)initWithTarget:(CBPeripheral * _Nonnull)target queue:(dispatch_queue_t _Nullable)queue OBJC_DESIGNATED_INITIALIZER;
